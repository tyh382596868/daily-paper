...
> **推荐做法**：直接运行 `backfill_links.py` 脚本完成 3b+3c，它会自动用相对路径生成
> **GitHub 和 Obsidian 双向可点**的 Markdown 链接，并把历史遗留的 `[[wikilink]]` 形式的
> 📒 行一并升级为可点链接：
>
> ```bash
...
Markdown 链接**（不要用 `[[wikilink]]`，因为 GitHub 上点不动）：

```markdown
- 📒 **笔记**: [笔记名](../论文笔记/分类目录/笔记名.md)
```

其中路径是从推荐文件（位于 `DailyPapers/`）到笔记文件的相对路径，所以以 `../论文笔记/` 开头；
路径里的空格写成 `%20`。这种相对 Markdown 链接在 GitHub 和 Obsidian 里都能点击跳转。

- 如果该论文已有 `📒 **已有笔记**` 或 `📒 **笔记**` 行，跳过不重复添加（但如果它仍是
  `[[wikilink]]` 形式，应升级为上面的可点链接）
