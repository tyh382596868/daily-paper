...
1. **Molmo2-ER 具身推理骨干**: 在 330 万具身样本上专化训练，13 个[[具身推理]]基准平均 63.8%，超越 GPT-5 和 Gemini Robotics ER-1.5
...
5. **[[逐层 KV 连接]]架构**: 动作专家每层直接访问对应深度 VLM 层的 KV 缓存，而非单一[[交叉注意力]]
...
现有 [[VLA|视觉-语言-动作模型]] 面临三大部署障碍：前沿模型闭源、开源模型绑定昂贵硬件、缺乏针对[[具身推理]]的专化视觉-语言骨干。

### 现有方法的局限

- [[π₀]]、[[Pi05|π₀.₅]] 等高性能 VLA 模型权重不开放或训练数据不公开
...
- **Backbone**: [[Molmo2]]-ER（[[具身推理]]专化的 VLM）
...
| 数据集 | 比例 | 机器人 | 动作表示 |
|--------|------|--------|----------|
| MolmoAct2-BimanualYAM | 30% | [[YAM 双臂机器人\|YAM]] | 绝对关节角 |
...
**说明**: 标准化的[[YAM 双臂机器人]] 遥操作数据采集设置，覆盖家用和工厂场景，共 34,500 条演示。这是目前最大的开源双臂操控数据集。
...
**说明**: MolmoAct2-DROID 在 MolmoSpaces（仿真 Pick/Place/Open/Close）和 MolmoBot（真实操控任务）上均大幅超越 [[Pi05|π₀.₅]]-DROID，平均成功率分别为 37.7% vs. 34.5%（MolmoSpaces）和 87.1%（MolmoBot）。
...
| [[Pi05\|π₀.₅]] | 98.8% | 98.2% | 98.0% | 92.4% | 96.9% |
...
| [[DROID 数据集\|MolmoAct2-DROID]] | 74.6K 集 | 质量筛选的 Franka 数据 | 训练 |
...
3. **架构创新有据可查**: [[逐层 KV 连接]]的逐层消融验证了设计的有效性
...
- [[Pi05|π₀.₅]]: 主要对比基线，MolmoAct2-Think 在 LIBERO 上超越
